...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		<?php the_content(); ?>
        <?php echo 'Ticket Price: ' . get_post_meta(get_the_ID(), 'ticket_price', true); ?>
        <br>
        <?php echo 'Film Release Date:' . get_post_meta(get_the_ID(), 'release_date', true); ?>
        <br>
        Countries: <ul><?php echo get_the_term_list( get_the_ID(), 'country', '<li class="jobs_item">', ', ', '</li>' ) ?></ul>
        Genres: <ul><?php echo get_the_term_list( get_the_ID(), 'genre', '<li class="jobs_item">', ', ', '</li>' ) ?></ul>
		<?php
			wp_link_pages( array(
